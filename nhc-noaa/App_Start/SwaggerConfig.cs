    ...
    public class SwaggerConfig
    {
                ...
                .EnableSwagger(c =>
                    {
                        ...
                        //
                        c.SingleApiVersion("v1", "nhc_noaa");
        ...
        private class ApplyDocumentVendorExtensions : IDocumentFilter
        {
            public void Apply(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
            {
                ...
                {
                    if (path.Value.get.parameters != null)
                    {
                        foreach (var param in path.Value.get.parameters)
                        {
                            if (param.format == "date-time")
                            {
                                ...
                                {
                                    case "min":
                                        param.@default = DateTime.Now.AddDays(-7);
                                        break;
                                    case "max":
                                        param.@default = DateTime.Now;
                                        break;
                                }
                            }
                        }
